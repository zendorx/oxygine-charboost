...
DECLARE_SMART(CharboostDialog, spCharboostDialog);
class CharboostDialog : public Actor
{
public:
    enum
    {
...
//Json::Value _charboost(Json::objectValue);

/*void charboostSimulatorLogin()
{
    spCharboostDialog dialog = new CharboostDialog;
    ...
    dialog->_btnOk->addClickListener([ = ](Event * e)
    {
        ...
        charboost::internal::loginResult(true);
    });
...
void charboostSimulatorInit()
{
    log::messageln("Charboost Simulator Init");


    /*file::buffer bf;
    file::read(".charboost", bf, ep_ignore_error);

    if (!bf.empty())
    {
        Json::Reader reader;
        reader.parse((char*)&bf.front(), (char*)&bf.front() + bf.size(), _charboost, false);
    }*/
}
